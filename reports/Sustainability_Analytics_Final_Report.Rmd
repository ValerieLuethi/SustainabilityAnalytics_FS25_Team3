...
## Alignment with UN Sustainable Development Goals [SDGs](https://www.un.org/sustainabledevelopment/sustainable-development-goals/)

This project contributes to several of the UN`s SDGs, particularly:
...
```{r rhone_river_data_preparation, include = FALSE}
# rename columns 
df_rhone <- raw_rhone %>%
  # rename(new_name = old_name)
  rename(
...
plot(
  ts_mass,
  ylab = "Yearly Aletsch Glacial Area",
  xlab = "",
  col = "blue",
  lwd = 1,
)
...
plot(
  waterlevel_ts,
  ylab = "Monthly Waterlevels of Massa",
  xlab = "",
  col = "blue",
  lwd = 1,
)
...
rain_ts <- ts(df_rain$rhs150m0, start = c(year(min(df_rain$Date)),
                                          month(min(df_rain$Date))), 
              frequency = 12)
summary(rain_ts)
...
```
Precipitation data for Brig reveal considerable short-term variability but no strong long-term trend. This relative stability indicates that changes in precipitation patterns are not the primary factor driving river level changes. Instead, glacier melt, mediated by rising temperatures, emerges as the dominant influence.

## Implications
Taken together, these findings highlight the interconnected impacts of climate change on Brig’s water resources:

  * Rising temperatures accelerate glacier retreat, reducing long-term ice reserves.
  
  * In the short term, this enhances Massa River water levels, increasing the risk of high summer flows and potential flooding.
  
  * In the long term, continued glacier shrinkage will weaken the buffering capacity of meltwater, threatening water security for Brig despite stable precipitation.
