...
Link to GitHub Repo: [SustainabilityAnalytics_FS25_Team3](https://github.com/ValerieLuethi/SustainabilityAnalytics_FS25_Team3)
...
To answer this question, our analysis is divided into three components:

- Climate-Glacier Model: We use an ARIMAX model with annual data to link climate variables (temperature and precipitation) to glacier mass balance to forecast long-term ice loss

- Climate-River Model (Monthly): We use a SARIMAX model with monthly data linking climate variables (temperature and precipitation) to river discharge to gain insights for wate rmanagement
...
- Jungfraujoch Temperature - Monthly air temperature from the high-alpine Jungfraujoch station
...
A detailed explanation of glacier mass balance and hydro-meteorological river data can be found in the [feature explanation](#Table_feature_exp) in the appendix.

<u>**Loading Data**</u>
```{r loading_data, include= TRUE, echo = TRUE, results = 'hide'}
# Reading in data
...
```
Additionally for later use in the annual Climate-Glacier model an annual time series was created by aggregating the monthly values to annual mean values.

<u>**data preparation (annual aggregation) for jungfraujoch**</u>
```{r jungfraujoch_temperature_data_preparation_annual, include = TRUE, echo = TRUE, results = 'hide'}
# create a year column
df_jungfraujoch_year <- df_jungfraujoch_reduced %>%
  mutate(Year = year(reference_timestamp))
...
```{r glacier_data_preparation, include = TRUE, echo = TRUE, results = 'hide'}
...
```{r glacier_monthly_data_preparation, include = TRUE, echo = TRUE, results = 'hide'}
...
```{r massa_river_data_preparation, include = TRUE, echo = TRUE, results = 'hide'}
# rename columns 
df_massa <- raw_massa %>%
...
```{r monthly_massa_river_data_preparation, include = TRUE, echo = TRUE, results = 'hide'}
# create monthly df
df_monthly <- df_massa %>%
  mutate(
...
```{r monthly_average_massa, include = TRUE,echo = TRUE, results = 'hide'}
...
```{r water_discharge_massa_preparation, include = TRUE, echo = TRUE, results = 'hide'}
# Read data
#df_massa <- raw_massa
...
```{r rhone_river_data_preparation, include = TRUE, echo = TRUE, results = 'hide'}
# rename columns 
df_rhone <- raw_rhone %>%
  # rename(new_name = old_name)
  rename(
...
```{r brig_rain_data_preparation, include = TRUE, echo = TRUE, results = 'hide'}
...
```{r brig_rain_annual aggregation, include = TRUE, echo = TRUE, results = 'hide'}
# create year column
...
lines(lowess(time(ts_jungfraujoch), ts_jungfraujoch), col = "red", lwd = 2)
...
```{r glacier_eda_ii, include = TRUE}
print("Summary Statistics of Winter mass balance (mmwe) values:")
summary(df_glacier$winter_mass_balance_mmwe)
print("Summary Statistics of Summer mass balance (mmwe) values:")
summary(df_glacier$summer_mass_balance_mmwe)
```
The summary statistics reveal a clear asymmetry between the two seasons: winter mass balance values are consistently positive, ranging from about 80 to 2100 mm w.e., indicating steady accumulation of snow and ice. By contrast, summer mass balance values are consistently negative, with losses reaching nearly –4000 mm w.e.
The seasonal mass balance suggests that summer ablation far outweighs winter accumulation, making glacier retreat inevitable under rising temperatures. We hypothesize that this imbalance could lead to higher short-term summer flows in the Massa River.
...
After differencing the plot shows no visible trend. The ACF decays quickly while the PACF shows a mean reversion (negative partial autocorrelation) with a remaining AR process. The ADF test no indicates stationarity as well (p = .01).

<u>**Jungfraujoch temperature annual after differencing**</u>
...
The ACF shows a strong seasonal pattern reflecting the seasonal temperature cycle. The PACF indicates that monthly mean temperatures are influenced by the previous month (lag 1). Then shows a negative connection for the next few months (lags 2 to 6) and a positive connection again (lags 8 to 13). This is likely due to temperature changes following a seasonal pattern, where each month corresponds to a specific period (e.g. season). Despite the clear seasonality in the monthly temperature the ADF test indicates stationarity (p = .01).
...
# adf
adf.test(ts_jungfraujoch)
```

After testing different windows for STL decomposition, we observed that larger seasonal windows slightly reduced autocorrelations in the remainder, whereas the PACF improved notably when using a large trend window. An s.window of 37 and a t.window of 121 showed the most satisfying result. The ADF test was used again to formally check stationarity of the remainder. The significant result (p < .01) indicates stationarity.
...
It looks like there is a potential downward trend starting around 2010. The ACF and PACF indicate stationarity, whereas the ADF test is not significant (p > .05) suggesting non-stationrarity. Since glacier-mass balance represents an annual physical balance (with fluctuations around ~ 0) we decided not to difference it. 
...
```
The original monthly glacier series had strong trend and seasonal components and was not stationary. After seasonal differencing (lag=12) the series appears stationary: 
...
For total annual precipiation the ACF and PACF as well as the ADF test indicate stationarity (p = .01).
...
# 
# # RDS export
saveRDS(massa_monthly_mean_stl, "../data/processed/stationary_monthly_mean_watervolume_massa.rds")
```


## Merge Stationary Data
...
# Glacier
df_glacier <- df_glacier %>%
  mutate(Date = as.Date(ym(year_month)))
...
df_merged_overlap <- df_merged %>%
  drop_na(remainder_jungfrau, remainder_glacier,
          remainder_waterlevel, remainder_waterdischarge,
...
## Climate-Glacier Model (Annual)

First we examine the relationships between climate variables (mean temperature and total precipitation) with glacier mass balance on a yearly level.
...
The top left plot shows how changes in the glacier mass-balance depend on the absolute (non-differenced) air temperature. The CCF is significant at lag 1. This indicates that the previous year's temperature affects the current glacier mass-balance. Since the relationship is negative, higher temperatures are associated with lower glacier mass-balance. Yet since CCF assumes stationarity the result should be interpreted with caution. When using the differenced temperature, the CCF shows no significant lags. This might indicate that year-to-year changes in temperature do not strongly drive glacier mass balance. Therefore for further modelling with ARIMAX temperature lagged once will be included in levels as an external factor.

<u>**CCF Temperature and Glacier mass balance**</u>
```{r ccf_temperature_glacier, include = TRUE, warning=FALSE, message=FALSE}
# option 1: temperature in levels
common_start2 <- max(start(ts_jungfraujoch_annual)[1], start(ts_glacier_annually)[1], start(rain_ts_annual)[1])
common_end2   <- min(end(ts_jungfraujoch_annual)[1],   end(ts_glacier_annually)[1],   end(rain_ts_annual)[1])

ts_temp_level_aligned <- window(ts_jungfraujoch_annual, start = common_start2, end = common_end2)
ts_glacier_level_aligned <- window(ts_glacier_annually, start = common_start2, end = common_end2)
ts_rain_level_aligned <- window(rain_ts_annual, start = common_start2, end = common_end2)

par(mfrow = c(1,2))
ccf(ts_glacier_level_aligned, ts_temp_level_aligned, main="Glacier & Temp (levels)")
ccf(ts_temp_level_aligned, ts_glacier_level_aligned, main="Temp (levels) & Glacier")

# option 2: differenced temperature

ccf(ts_glacier_level_aligned, diff(ts_temp_level_aligned), main="Glacier & Temp (diff)")
ccf(diff(ts_temp_level_aligned), ts_glacier_level_aligned, main="Temp (diff) & Glacier")

```

We also check the correlation between the temperature lagged once with the glacier mass balance, which is relatively weak (r = -0.19). While temperature has some effect, other factors likely influence the glacier mass balance as well.

<u>**Correlation: Temperature lag 1 with Glacier mass balance**</u>
```{r temperature_lagged_correlation, include = TRUE, warning=FALSE, message=FALSE}
# Lag temperature by 1 year
temp_lag1 <- stats::lag(ts_temp_level_aligned, -1)
# cor(ts_glacier_level_aligned, ts_temp_level_aligned)
cor(ts_glacier_level_aligned, temp_lag1, use="complete.obs")
```

#### Precipitation vs. Glacier

We now look at the CCF between total annual precipiation and glaicer mass balance. The left plot shows how changes in precipitations might affect glacier mass-balance. The CCF displays no significant lags, indicating no relationship between the two variables. The correlation (r = -0.04) is very weak as well. Since the precipitation was measured in the valley in Brig, it likely doesn't reflect the snow accumulation on the glacier due to substantial elevation differences. As glaciers respond primarily to seasonal snowfall (in winter), using total annual precipitation may not provide sufficient temporal resolution to capture this effect.

<u>**CCF Precipitation and Glacier mass balance**</u>
```{r correlation_precipitation_glacier, include = TRUE, warning=FALSE, message=FALSE}
par(mfrow = c(1, 2))
# Precipitation affecting glacier balance 
ccf(ts_glacier_level_aligned, ts_rain_level_aligned, main="Glacier & Precipitation")
ccf(ts_rain_level_aligned, ts_glacier_level_aligned, main="Precipitation & Glacier")
```

<u>**Correlation: Precipitation with Glacier mass balance**</u>
```{r ccf_precipitation_glacier, include = TRUE, warning=FALSE, message=FALSE}
# correlation
cor(ts_glacier_level_aligned, ts_rain_level_aligned)
```

### ARIMAX

We chose to use an ARIMAX model to account for the autocorrelation in the glacier mass balance time series while incorporating temperature as an exogenous variable. This approach allows us to quantify and forecast the influence of climate variables on glacier mass changes over time. Total annual precipitation was not included, as it appeared to explain no variance in glacier mass-balance.

We applied auto.arima() to the glacier mass-balance time series, suggesting an ARIMA(0,0,3) model with non-zero mean.

<u>**Auto Arima**</u>
```{r auto.arima_glacier_mass_balance, include = TRUE, warning=FALSE, message=FALSE}
# determine ARIMA order
fit.auto <- auto.arima(ts_glacier_level_aligned, stationary=TRUE, seasonal=FALSE)
summary(fit.auto)
```

<u>**Auto Arima Residual Check**</u>
```{r auto.arima_residuals_glacier_mass_balance, echo=TRUE, warning=FALSE, message=FALSE}
checkresiduals(fit.auto)
```

An exhaustive grid search over AR, differencing, and MA orders indicated ARIMA(2,1,2) had a slightly lower AIC, but the improvement was minimal. Given the simplicity and stationary residuals of the ARIMA(0,0,3) model, we chose it for further analysis.

<u>**Grid search for model order**</u>
```{r arima_grid_search_glacier_mass_balance, include = TRUE, results = 'hide', warning=FALSE, message=FALSE}
# determine ARIMA order
best_aic <- Inf
best_order <- c(0,0,0)
...
Next we included the lagged temperature as an exogeneous variable in the ARIMAX. The model estimates the effect of temperature on glacier mass balance while taking the MA(3) autocorrelation into account. 
...
$$
The negative coefficient for temperature indicates that higher previous-year temperatures tend to be associated with lower glacier mass balance. The moving average terms account for short-term dependencies in glacier mass balance residuals, meaning past shocks influence the current value. 

<u>**ARIMAX Glacier Model with Temperature**</u>
```{r arimax_glacier_model, include = TRUE, warning=FALSE, message=FALSE}
# remove first observation because of lagging
glacier_model_data <- ts_glacier_level_aligned[-1]
...
```
For the model fit the residual autocorrelation is low (ACF1 ~ 0.033) and training errors are reasonable (RMSE ~ 565 mm). The visual inspection of the residuals suggests that the model captures temporal structures and the effect of temperature adequately. Plotting the actual observations against the fitted values shows that the ARIMAX captures the main trend in glacier mass balance influenced by previous-year temperature.
...
To assess future glacier mass balance of Aletsch Glacier under different climate scenarios we used temperature projections for Jungfraujoch derived from the MeteoSchweiz report "Klimaszenarien Schweiz – eine regionale Übersicht" which is based on the Swiss Climate Scenarios CH2011 ([MeteoSchweiz](https://www.nccs.admin.ch/nccs/de/home/klimawandel-und-auswirkungen/schweizer-klimaszenarien/schweizer-klimaszenarien-im-ueberblick.html#-891137887)).These scenarios provide regional temperature projections for 2060 under different warming assumptions (low, medium and high). 

By 2060, glacier mass balance at Jungfraujoch is projected to be negative under all warming scenarios, indicating continued ice loss. Cumulative losses increase with warming: −41,265 mm w.e. for low, −45,641 mm w.e. for medium, and −50,018 mm w.e. for high warming. Even moderate temperature increases (2–3 °C) result in substantial retreat, while higher warming accelerates mass loss. These results highlight the sensitivity of glaciers to temperature changes and the importance of climate mitigation for preserving alpine ice resources.

<u>**Climate Scenario-Based Projections**</u>
```{r climate_scenario_based_projections, include = TRUE, warning=FALSE, message=FALSE}
# Historical baselines
jungfraujoch_historical <- data.frame(
  period_1961_1990 = c(winter = -13.6, summer = -1.8),
  period_1981_2010 = c(winter = -12.8, summer = -0.8),
  projection_2060 = c(winter_low = -11.1, winter_high = -9.4, 
                     summer_low = 1.5, summer_high = 3.5)
)

# Calculate annual means
annual_1961_1990 <- mean(c(-13.6, -1.8))  # -7.7°C
annual_1981_2010 <- mean(c(-12.8, -0.8))  # -6.8°C
annual_2060_low <- mean(c(-11.1, 1.5))    # -4.8°C
annual_2060_high <- mean(c(-9.4, 3.5))    # -2.95°C

# Current baseline
current_jungfraujoch <- as.numeric(tail(ts_temp_level_aligned, 1))

# Time steps from current to 2060
current_year <- 2024  # end of current ts
target_year <- 2060
years_to_project <- target_year - current_year # 36 years

# Linear warming scenarios
warming_low <- annual_2060_low - annual_1981_2010   # Conservative warming
warming_high <- annual_2060_high - annual_1981_2010 # High warming

# temperature projections
future_years <- 1:years_to_project
temp_scenario_low <- current_jungfraujoch + (warming_low / years_to_project) * future_years
temp_scenario_high <- current_jungfraujoch + (warming_high / years_to_project) * future_years
temp_scenario_medium <- (temp_scenario_low + temp_scenario_high) / 2

# Create lagged temperature matrices for each scenario
scenarios <- list(
  conservative = c(tail(ts_temp_level_aligned, 1), temp_scenario_low[-length(temp_scenario_low)]),
  medium = c(tail(ts_temp_level_aligned, 1), temp_scenario_medium[-length(temp_scenario_medium)]),
  high_warming = c(tail(ts_temp_level_aligned, 1), temp_scenario_high[-length(temp_scenario_high)])
)

# Generate projections
projections <- list()
for(scenario_name in names(scenarios)) {
  projections[[scenario_name]] <- forecast(arimax_glacier, 
                                          h = years_to_project,
                                          xreg = scenarios[[scenario_name]])
}

# Get the actual years from your historical data
hist_years <- as.numeric(time(ts_glacier_level_aligned))
forecast_years <- seq(max(hist_years) + 1, target_year, by = 1)

# Create extended time series
all_years <- c(hist_years, forecast_years)
hist_data <- as.numeric(ts_glacier_level_aligned)

# Custom plot with proper year labels
plot(all_years[1:length(hist_data)], hist_data, 
     type = "l", xlim = c(min(all_years), max(all_years)),
     ylim = c(-2000, 500), col = "black", lwd = 2,
     main = "Glacier Balance Climate Scenarios to 2060",
     xlab = "Year", ylab = "Mass Balance (mm w.e.)")

# Add forecast scenarios
lines(forecast_years, projections$medium$mean, col = "blue", lwd = 2)
lines(forecast_years, projections$conservative$mean, col = "green", lwd = 2)
lines(forecast_years, projections$high_warming$mean, col = "red", lwd = 2)

# Add vertical line at forecast start
abline(v = max(hist_years), lty = 2, col = "gray")

legend("topright", c("Medium Warming", "Conservative", "High Warming"), 
       col = c("blue", "green", "red"), lty = 1)

# Summary of projections
final_projections <- data.frame(
  Scenario = c("Low Warming", "Medium Warming", "High Warming"),
  Temp_Change_by_2060 = c(round(warming_low, 1), 
                          round(mean(c(warming_low, warming_high)), 1),
                          round(warming_high, 1)),
  Final_Balance_2060 = round(c(tail(projections$conservative$mean, 1),
                              tail(projections$medium$mean, 1),
                              tail(projections$high_warming$mean, 1)), 0),
  Cumulative_Loss = round(c(sum(projections$conservative$mean[projections$conservative$mean < 0]),
                           sum(projections$medium$mean[projections$medium$mean < 0]),
                           sum(projections$high_warming$mean[projections$high_warming$mean < 0])), 0)
)
print(final_projections)
```

The 95% confidence intervals (CIs) around the glacier mass balance projections represent statistical uncertainty in the ARIMAX model estimates. They account for variability in the model parameters and residuals, providing a range in which the true glacier mass balance is likely to fall. In our case the CIs are quite wide for all three scenarios, therefore conclusions should only be drawn with caution.

<u>**Confidence Intervalls**</u>
```{r confidence_intervals, include = TRUE, warning=FALSE, message=FALSE}
# Years for the forecast
forecast_years <- seq(max(as.numeric(time(ts_glacier_level_aligned))) + 1, 2060, by = 1)

# Set up 1 row x 3 columns for the panels
par(mfrow = c(1, 3), mar = c(4, 4, 3, 1))  # adjust margins

# Function to plot a scenario with CI
plot_scenario_CI <- function(proj, scenario_name, color) {
  plot(forecast_years, proj$mean, type = "l", col = color, lwd = 2,
       ylim = range(c(proj$lower[,2], proj$upper[,2])),
       main = scenario_name,
       xlab = "Year", ylab = "Mass Balance (mm w.e.)")
  
  # Add 95% CI as shaded polygon
  polygon(c(forecast_years, rev(forecast_years)),
          c(proj$lower[,2], rev(proj$upper[,2])),
          col = adjustcolor(color, alpha.f = 0.2),
          border = NA)
  
  # Add mean line on top
  lines(forecast_years, proj$mean, col = color, lwd = 2)
}

# Plot each scenario in its panel
plot_scenario_CI(projections$conservative, "Low Warming", "green")
plot_scenario_CI(projections$medium, "Medium Warming", "blue")
plot_scenario_CI(projections$high_warming, "High Warming", "red")
...
jungfrau <- as.numeric(ts_multivariate[, "remainder_jungfrau"])
glacier  <- as.numeric(ts_multivariate[, "remainder_glacier"])

# Plot scaled series
plot(scale(jungfrau), type = "l", col = "orange", lwd = 2,
     ylab = "Scaled value", xlab = "Time",
     main = "Scaled Jungfraujoch Temperature vs Glacier Mass Balance")
lines(scale(glacier), col = "blue", lwd = 2)
...
ccf(glacier, jungfrau, # temperature leads
    main = "Glacier vs Temperature")
# CCF: Glacier leads Temperatues
ccf(jungfrau, glacier, # glacier leads
    main = "Temperature vs Glacier")
```

#### Glacier vs. Massa River

The same argumentation as above applies to the cross-correlation function between glacier mass balance and the waterlevel and waterdischarge of the river Massa.
...
waterlevel <- as.numeric(ts_multivariate[, "remainder_waterlevel"])
glacier  <- as.numeric(ts_multivariate[, "remainder_glacier"])

# Plot scaled series
plot(scale(waterlevel), type="l", col="orange", lwd=2,
     ylab="Scaled value", xlab="Time")
lines(scale(glacier), col="blue", lwd=2)
...
ccf(glacier, waterlevel, # waterlevel leads
    main = "Glacier vs Waterlevel")
# CCF: Glacier leads Waterlevel
ccf(waterlevel, glacier, # glacier leads
    main = "Waterlevel vs Glacier")
...
# Scale the two series and convert to numeric vectors
waterdischarge <- as.numeric(ts_multivariate[, "remainder_waterdischarge"])
glacier <- as.numeric(ts_multivariate[, "remainder_glacier"])

# Plot scaled series
plot(scale(waterdischarge), type="l", col="orange", lwd=2,
     ylab="Scaled value", xlab="Time")
lines(scale(glacier), col="blue", lwd=2)
...
par(mfrow = c(1, 2), mar = c(4, 4, 5, 1)) # bottom, left, top, right
# CCF: Waterlevel leads Glacier
ccf(glacier, waterdischarge, # waterdischarge leads
    main = "Glacier vs Waterdischarge")
# CCF: Glacier leads Waterlevel
ccf(waterdischarge, glacier, # glacier leads
    main = "Waterdischarge vs Glacier")
# Reset plotting parameters
par(mfrow = c(1, 1), mar = c(5, 4, 4, 2) + 0.1)
```
#### Temperature vs. Massa River

For the cross-correlation function between the Jungfraujoch temperature and the waterlevels of Massa we can observe that temperature is leading with a weak yet significant positive correlations at lag 0.This indicates that higher temperatures are associated with slightly higher water levels in the same month, although the effect is small.
...
plot(scale(jungfrau), type="l", col="orange", lwd=2,
     ylab="Scaled value", xlab="Time")
lines(scale(waterlevel), col="blue", lwd=2)
...
ccf(waterlevel, jungfrau, 
    main = "Waterlevel vs Temperature")
# CCF: Glacier leads Temperatues
ccf(jungfrau, waterlevel,
    main = "Temperature vs Waterlevel")
```

When considering water discharge instead of water level, the correlation at lag 0 is stronger. This indicates that the monthly water discharge of Massa responds directly and quickly to temperature changes. Higher temperatures lead to increased discharge, reflecting the immediate hydrological response from snow and glacier melt, while lower temperatures correspond to reduced discharge due to reduced melt and freezing.
...
plot(scale(jungfrau), type="l", col="orange", lwd=2,
     ylab="Scaled value", xlab="Time")
lines(scale(waterdischarge), col="blue", lwd=2)
...
ccf(waterlevel, precipitation, 
    main = "Waterlevel vs Precipitation")
# CCF: Waterlevel leads Precipitation
ccf(precipitation, waterlevel,
    main = "Precipitation vs Waterlevel")
# Reset plotting parameters
par(mfrow = c(1, 1), mar = c(5, 4, 4, 2) + 0.1)
```
If we look at the water discharge instead, the CCF indicates significance at lag -5 and lag 6. This would mean that water discharge responds with a delay to precipitation (6 months lag) this could be due to snow melt. The correlation at lag -5 where water discharge leads precipitation might be spurious or due to seasonal patterns.
...
ccf(waterdischarge, precipitation, 
    main = "Waterdischarge vs Precipitation")
# CCF: Waterdischarge leads Precipitation
ccf(precipitation, waterdischarge,
    main = "Precipitation vs Waterdischarge")

```

### ARIMAX

For the ARIMAX we decided to focus on water discharge rather than water level, since it is more directly influenced by climate variables. 

As a first step we applied auto.arima() to the waterdischarge time series, suggesting an ARIMA(0,0,1) model. An inspection of the residuals implied stationarity.

<u>**Auto Arima**</u>
```{r auto_arima, include = TRUE, warning=FALSE, message=FALSE}
# auto arima for water discharge
fit.auto <- auto.arima(waterdischarge, stationary = TRUE, seasonal = TRUE)
summary(fit.auto)
```
<u>**Auto Arima Resiudal Check**</u>
```{r auto_arima_residuals, include = TRUE, warning=FALSE, message=FALSE}
checkresiduals(fit.auto)
```
We then included temperature (current month) and precipitation (lag 6) as exogeneous variables in the ARIMAX model to account for the immediate effect of temperature and the delayed hydrological response in precipitation. The model otuput suggests that higher temperatures in the current month lead to higher water discharge. This may reflect snow or ice melt, faster snowpack runoff, or seasonal processes that increase flow during warmer months. Precipitation from 6 months ago shows a statistically significant association with current discharge, but the effect size is very small and likely reflects seasonal patterns rather than a direct hydrological mechanism.

$$
\text{Water Discharge}_t = −0.0367 
+ 1.1547 \cdot \text{Temperature}_{t} 
+ 0.003 \cdot \text{Precipitation}_{t-6} 
+ \varepsilon_t
+ 0.1433\,\varepsilon_{t-1}
$$

<u>**ARIMAX**</u>
```{r ARIMAX, include = TRUE, warning=FALSE, message=FALSE}
# align ts and lag precipitation
# Lag precipitation by 6 (current month is lag 0)
precip_lag6 <- dplyr::lag(precipitation, 6)  # negative lag shifts forward

# Align series and remove NAs caused by lag
complete_idx <- complete.cases(waterdischarge, jungfrau, precip_lag6)
discharge_clean <- waterdischarge[complete_idx]
temp_clean <- jungfrau[complete_idx]          # lag 0, current month
precip_clean <- precip_lag6[complete_idx]

# -----------------------------
# 3) Build ARIMAX model with temperature lag 0 and precipitation lag 6
# -----------------------------
arimax_monthly <- Arima(discharge_clean,
                        order = c(0,0,1),      # From auto.arima
                        xreg = cbind(temp_clean, precip_clean),
                        include.constant = TRUE)
rmse_val <- accuracy(arimax_monthly)[1,"RMSE"] # For dynamic text
summary(arimax_monthly)
```

The residuals have very low autocorrelation (ACF1 ~ 0.014), indicating that the model captures the main temporal structure of the series. The root mean squared error (RMSE ~ `r round(rmse_val, 2)`) indicates a reasonable average deviation between the observed and fitted values. Plotting the actual observations against the fitted values shows the ARIMAX model captures the main trend in water discharge. Yet fitted values are relatively flat compared to the observed variability. This reflects the limited ability of the ARIMAX to reproduce extreme fluctuations. 

<u>**ARIMAX model fit**</u>
```{r ARIMAX_model_fit, include = TRUE, warning=FALSE, message=FALSE}
# check residuals
checkresiduals(arimax_monthly)
# Plot fitted vs actual
par(mfrow = c(1, 1))
plot(discharge_clean, main="Water discharge: Actual vs Fitted")
lines(fitted(arimax_monthly), col="red")
legend("topright", c("Actual", "Fitted"), col=c("black", "red"), lty=1)
```

When using the ARIMAX to forecast the next 12 months we see a continuation of the main trend.

<u>**ARIMAX forecast**</u>
```{r ARIMAX_forecast, include = TRUE, warning=FALSE, message=FALSE}
# forecast next 12 months
horizon <- 12
future_temp <- rep(mean(temp_clean), horizon)        
future_precip <- rep(mean(precip_clean), horizon) 

forecast_values <- forecast(arimax_monthly,
                            h = horizon,
                            xreg = cbind(future_temp, future_precip))

# Plot forecast
plot(forecast_values, main = "ARIMAX Forecast for Water Discharge",
     ylab = "Water Discharge", xlab = "Time")
lines(discharge_clean, col = "black", lwd = 2)

```

Since the ARIMAX is not suitable to assess the risk and frequency of rare high-discharge events, we applied extreme value theory (EVT) in our further analysis.
...
This report examined how climate change is reshaping the Aletsch Glacier and the Massa River that supports the town of Brig. By combining historical climate data, statistical forecasting, and extreme value theory, the analysis provides insights into future glaicer melt, water availability and flood risk. 

The results show that temperature is a main driver of glacier loss. Glacier mass balance responds to the previous year’s temperature, suggesting a one-year “climate memory.” The ARIMAX model shows a negative association between lagged temperature and glacier mass balance, meaning warmer years tend to coincide with lower glacier mass, after accounting for autocorrelated fluctuations in the series. Precipitation, as measured in the valley, has little effect.
...
Several limitations affect the interpretation and scope of this study, but they also highlight opportunities for future research:

  * *Temporal scale mismatch:* we used separate models to forecast annual glacier changes and monthly river flow, instead of one combined system. This prevents us from directly linking glacier melt to river discharge and limits quantification of the effect of glacier retreat on water supply. Future work should adopt integrated glacier–hydrology models.
  
  * *Spatial representativeness:* climate data from Jungfraujoch and Brig may not reflect the full range of glacier conditions. To improve coverage it would be beneficial to include data from additional weather stations and to take into account gridded climate data for further spatial analysis.
  
  * *Model assumptions:* ARIMAX assumes linear climate–glacier relationships, while glacier processes are complex and non-linear. Combining statistical and process-based models would better capture feedbacks, thresholds, and ice dynamics.
  
  * *Forecasting uncertainty:* our forecast for glacier melt is based on three different temperature scenarios, which account for part of the uncertainty in climate conditions. Yet they assume that past climate-glacier relationships will hold under extreme warming. Future research should continue to use multiple scenarios, expand uncertainty ranges and regularly update forecasts.
  
  * *Data gaps:* the current analysis would be greatly enhanced by data from the Massa hydro power project. This would help us identify how much artificial regulation of the Massa water levels is performed by the dam. Using this, we can extrapolate the true effect of the melting ice from the glacier on the river.
  
  * *Additional climate drivers:* the project would benefit by extending the analysis to include other contributing factors such as solar radiation, greenhouse gas emissions and local topography such as slope and ascent. All these factors directly affect glacier melt rate thereby adding to rising water levels in the rivers.
  
  * *Long-term perspective:* considering a long-term plan, including research from other climate studies would be crucial in improving model accuracy by provide more context on global warming trends, comparative data and analyzing general patterns of how glacier melts affect rivers. 
  
  * *Regional expansion:* for a future course of action, the analysis can be extended to other glaciers in Switzerland, and even further to the Alps as a whole. Owing to the large size and surface area of Aletsch glacier, it is more resilient to changing environmental conditions. Hence, studying other glaciers might show evidence of more pressing concerns.
