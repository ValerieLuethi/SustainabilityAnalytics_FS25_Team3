...
In order to convert the data into a time series later on, we first converted the reference time stamp into the datetime format. We discarded all columns that were not necessary for our analysis and renamed the remaining columns. The measurements of the homogeneous monthly mean air temperature were complete for the whole period (no missings). In the last step we converted the data into a time series object.
...
A frist inspection of the summary statistics shows a minimum monthly mean temperature of -21.7 C° and a maximum monthly mean temperature of 3.5 C°. The mainly negative values align with the expected cold climate on Jungfraujoch. Yet the the difference between the coldest and warmest value shows a substantial difference around 25 C°.
The visual inspection shows strong oscillations representing the expected seasonality in temperature data. There seems to be a slight upwards trend as well.
The ACF of the time series shows a clear seasonal pattern where cold winters alternate with relatively mild summers. The PACF indicates strong autogregressive components, meaning each month's mean temperature is influenced by preceding months. This reflects characteristics we would expect in climate data.
...
# plot
hist(ts_jungfraujoch, main="Distribution Jungfraujoch Monthly Temperature", ylab="C°")
boxplot(ts_jungfraujoch, main="Boxplot Jungfraujoch Monthly Temperature", ylab="C°")
