...
<u>**CCF Annual Data**</u>
```{r ccf_annual_data include = TRUE, warning=FALSE, message=FALSE}
...
# Verify they're now the same length
length(ts_temp_aligned)
length(ts_glacier_aligned)
length(ts_precip_aligned)

# Temperature changes affecting glacier balance
ccf(ts_temp_aligned, ts_glacier_aligned)
# Precipitation affecting glacier balance  
ccf(ts_precip_aligned, ts_glacier_aligned)
# Also check reverse direction (less likely but good to verify)
ccf(ts_glacier_aligned, ts_temp_aligned)
ccf(ts_glacier_aligned, ts_precip_aligned)
```
...
auto.arima(ts_glacier_aligned, stationary=TRUE, seasonal=FALSE)
# Fit ARIMA(0,0,3) (MA(3))
fit_ma3 <- Arima(ts_glacier_aligned, order = c(0,0,3), include.mean = TRUE)
summary(fit_ma3)
# check residuals
checkresiduals(fit_ma3)
# ACF and PACF of residuals
par(mfrow=c(2,1))
acf(residuals(fit_ma3))
pacf(residuals(fit_ma3))
# forecast
h <- 10 # forecast horizon in years
fc_ma3 <- forecast(fit_ma3, h = h)
autoplot(fc_ma3) + ggtitle("ARIMA(0,0,3) forecast: Glacier mass balance")
print(fc_ma3)   # view mean and intervals
```
...
precip_data <- ts_precip_aligned[-1]

# ARIMAX
arimax_glacier <- Arima(glacier_model_data,
                        order = c(0,0,3), 
                        xreg = cbind(temp_lag1_data, precip_data))
# check model
...
forecast(arimax_glacier, h=5, 
         xreg=cbind(tail(temp_lag1_data, 5), tail(precip_data, 5)))
```
Glacier Balance(t) = -2372.17 - 270.95×Temperature(t-1) - 0.018×Precipitation(t) + MA errors
