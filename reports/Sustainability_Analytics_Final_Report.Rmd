...
```{r jungfraujoch_aggregate_annual_mean, include = TRUE, echo = TRUE, results = 'hide'}
...
It looks like there is a potential downward trend starting around 2010. The ACF and PACF indicate stationarity, whereas the Augmented Dickey-Fuller test is not significant (p > .05) suggesting non-stationrarity. Since glacier-mass balance represents an annual physical balance (with fluctuations around ~ 0) we decided not to difference it. 
...
# common_start <- max(start(ts_temp_diff)[1], start(ts_glacier_annually)[1], start(rain_ts_annual)[1])
# common_end <- min(end(ts_temp_diff)[1], end(ts_glacier_annually)[1], end(rain_ts_annual)[1])
# 
# # Extract common periods
# ts_temp_aligned <- window(ts_temp_diff, start=common_start, end=common_end)
...
# -----------------------------
# Option 1: Temperature in levels (non-differenced)
# -----------------------------
...
The left plot shows how changes in the glacier mass-balance depend on the absolute (non-differenced) air temperature. The CCF is significant at lag 1. This indicates that the previous year's temperature affects the current glacier mass-balance. Since the relationship is negative, higher temperatures are associated with lower glacier mass-balance. Yet the correlation (r = -0.19) is relativley weak and since ccf assumes stationarity the result should be interpreted with caution. When using the differenced temperature, the ccf shows no significant lags. This might indicate that year-to-year changes in temperature do not strongly drive glacier mass balance. Therefore for further modelling with ARIMAX temperature will be included in levels as an external factor.
...
temp_model <- auto.arima(ts_temp_level_aligned)
...
last_temp <- tail(ts_temp_level_aligned, 1)
...
The climate projections for Jungfraujoch are from the MeteoSchweiz report "Klimaszenarien Schweiz – eine regionale Übersicht" which is based on the Swiss Climate Scenarios CH2011.
...
<u>**Simulation form ARIMAX**</u>
```{r simulation_arimax, include = TRUE, warning=FALSE, message=FALSE}
library(forecast)
...
# Run ensemble simulations for each scenario
set.seed(123)
for (sc in names(scenarios)) {
  sim_mat <- matrix(NA, nrow = h, ncol = nsim)
  for (i in seq_len(nsim)) {
    sim_mat[, i] <- as.numeric(simulate(
      arimax_glacier, nsim = h, xreg = scenarios[[sc]], bootstrap = TRUE
    ))
  }
  scenario_qs[[sc]] <- apply(sim_mat, 1, quantile, probs = probs, na.rm = TRUE)
}

# --- Plot ---
plot(all_years[1:length(hist_data)], hist_data,
     type = "l", col = "black", lwd = 2,
     xlim = c(min(all_years), max(all_years)),
     ylim = c(
       min(unlist(scenario_qs), hist_data, na.rm = TRUE),
       max(unlist(scenario_qs), hist_data, na.rm = TRUE)
     ),
     main = "Glacier Balance Climate Scenarios to 2060",
     xlab = "Year", ylab = "Mass Balance (mm w.e.)")
...
# Plot each scenario with shaded fan
for (sc in names(scenarios)) {
  ...
  polygon(c(forecast_years, rev(forecast_years)),
          c(q["25%", ], rev(q["75%", ])),
          col = adjustcolor(col, alpha.f = 0.3), border = NA)
  ...
  # Median line
  lines(forecast_years, q["50%", ], col = col, lwd = 2)
}

legend("topright",
       legend = c("Historical", "Conservative", "Medium", "High warming"),
...
```
- precipitation leading at lag 6 --> dealyed run-off response due to snow accumulation and delayed melt?
- waterdischarge leading at lag 5? doesn't make much sense

###ARIMAX

<u>**Identify ARIMA structure**</u>
```{r auto_arima, include = TRUE, warning=FALSE, message=FALSE}
# -----------------------------
# 1) Auto ARIMA for water discharge
# -----------------------------
...
# -----------------------------
# 2) Align series and remove NAs (if any)
# -----------------------------
complete_idx <- complete.cases(waterdischarge, jungfrau, precipitation)
discharge_clean <- waterdischarge[complete_idx]
temp_clean <- jungfrau[complete_idx]
precip_clean <- precipitation[complete_idx]

# -----------------------------
# 3) Build ARIMAX model with current precipitation
# -----------------------------
...
# -----------------------------
par(mfrow = c(1, 1))
fitted_values <- fitted(arimax_monthly)

plot(discharge_clean, type = "p", col = "black", pch = 16,
     ylab = "Water Discharge", xlab = "Time",
     main = "Observed vs Fitted Water Discharge")

lines(fitted_values, col = "red", lwd = 2)  # fitted as red line

legend("topright", legend = c("Observed", "Fitted"),
       col = c("black", "red"), pch = c(16, NA), lty = c(NA, 1), lwd = 2)

# -----------------------------
# 5) Forecast future discharge
# -----------------------------
# Example: forecast next 12 months
horizon <- 12
future_temp <- rep(mean(temp_clean), horizon)        # replace with scenario if available
future_precip <- rep(mean(precip_clean), horizon)   # replace with scenario if available
...
plot(forecast_values, main = "ARIMAX Forecast for Water Discharge",
     ylab = "Water Discharge", xlab = "Time")
lines(discharge_clean, col = "black", lwd = 2) # add historical
```
