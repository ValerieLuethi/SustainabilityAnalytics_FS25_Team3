...
It looks like there is a potential downward trend starting around 2010. The ACF and PACF as well as the Augmented Dickey-Fuller test indicate stationarity.
...
# 
# # RDS export
saveRDS(massa_monthly_mean_stl, "stationary_monthly_mean_watervolume_massa.rds")
```
...
# Current baseline
current_jungfraujoch <- as.numeric(tail(ts_temp_aligned, 1))
...
scenarios <- list(
  conservative = c(tail(ts_temp_aligned, 1), temp_scenario_low[-length(temp_scenario_low)]),
  medium = c(tail(ts_temp_aligned, 1), temp_scenario_medium[-length(temp_scenario_medium)]),
  high_warming = c(tail(ts_temp_aligned, 1), temp_scenario_high[-length(temp_scenario_high)])
)
